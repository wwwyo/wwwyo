...
唯一のアクセントは Deep Persimmon `{colors.primary}`（ダークでは明度を上げた Apricot `#F08849`）。HN の `#ff6600` より深く沈めた柿色で、白地に対して WCAG AA（4.5:1 以上）を確保している。使ってよい場所は3つだけ: (1) リンクの下線、(2) hover 時のリンク文字色、(3) 見出し・`<hr>` などの構造マーカーの点使い。地の色や広い面には決して使わない。
